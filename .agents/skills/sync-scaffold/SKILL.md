...
The split between upstream-tracked and instance-only files is authoritative in [`UPSTREAM.manifest.yml`](../../../UPSTREAM.manifest.yml), with prose context in [`UPSTREAM.md`](../../../UPSTREAM.md). Consult both before doing anything.
...
Read `UPSTREAM.manifest.yml` and use only `upstream_tracked.exact` plus `upstream_tracked.globs` as the candidate path set. `template_seed_instance_files`, `instance_only`, `overlay_extension_paths`, `sync_exclusions`, and `ignored_private_paths` are not candidates for replacement in an instantiated family KB.
...
**Never include any path outside the manifest's upstream-tracked list.** If `git diff upstream/main` shows changes to an instance-only or seed-instance file, that means the upstream drifted from this instance in a way that touches real content — report it to the user and stop; do not apply.
...
Explicit "never touch" paths are defined in `UPSTREAM.manifest.yml` under `template_seed_instance_files`, `instance_only`, `overlay_extension_paths`, and `sync_exclusions`. The skill refuses to include these in any diff or apply step. Important examples:
...
- `.agents/skills.overlay/**` (gitignored; host-app-injected skills).
- `.agents/hooks.overlay/**` (gitignored; host-app-injected hook scripts or descriptors).
- `.agents/settings.local.json` (gitignored; generic host-app settings).
